...
var Article = keystone.list('LivingArticle');
var ArticleComment = keystone.list('LivingArticleComment');
...
exports = module.exports = function(req, res) {
		...
		Article.model.findOne()
			...
			.exec(function(err, article) {
				...
				if (article.state == 'published' || (req.user && req.user.isAdmin) || (req.user && article.author && (req.user.id == article.author.id))) {
					...
					locals.page.title = article.title + ' - Living - Babylon';
				} else {
	...
	// Render the view
	view.render(keystone.lang + '/site/livingOne');
