...
exports = module.exports = function(req, res) {
	...
	view.on('init', function(next) {
		...
		keystone.list('ThingsToDoArticleTag').model.find({'language': req.cookies.lang}).sort('name').exec(function(err, results) {
	...
	view.on('init', function(next) {
		...
		if (req.params.tag) {
			keystone.list('ThingsToDoArticleTag').model.findOne({$and:[{'language': req.cookies.lang}, { key: locals.filters.tag }]}).exec(function(err, result) {
	...
	view.on('init', function(next) {
		...
		var q = keystone.list('ThingsToDoArticle').model.find().where({$and:[{'state':'published'}, {'country': cookie}, {'language': lang}]}).sort('-publishedDate').populate('author tags');
	...
	view.render(lang + '/site/things_to_do');
