...
exports = module.exports = function(req, res) {
	...
	view.on('init', function(next) {
		...
		if(req.query.search){ // if the URL contains ?search=something : load the specific news 
			...
			var q = keystone.list('Post').model.find({$or: [ {title: {$regex: '.*' + req.query.search + '.*', $options: 'i'}},{'content.extended': {$regex: '.*' + req.query.search + '.*', $options: 'i'}}]}).where({$and:[{'state':'published'}, {'country': cookie}]}).sort('-publishedDate').populate('author tag');
			...
			var q = keystone.list('Post').model.find().where({$and:[{'state':'published'}, {'country': cookie}]}).sort('-publishedDate').populate('author tag');
