...
var Article = keystone.list('PlanYourTripArticle');
var ArticleComment = keystone.list('PlanYourTripArticleComment');
...
exports = module.exports = function(req, res) {
		...
		Article.model.findOne()
			...
			.exec(function(err, article) {
				...
				if (article.state == 'published' || (req.user && req.user.isAdmin) || (req.user && article.author && (req.user.id == article.author.id))) {
					...
					locals.page.title = article.title + ' - Plan your trip - Babylon';
				} else {
	...
	view.on('post', { action: 'create-comment' }, function(next) {
		...
		updater.process(req.body, {
			...
			if (err) {
				...
				return res.redirect('/plan_your_trip/article/' + locals.article.slug);
			}
			next();
		});
	...
	// Render the view
	view.render(keystone.lang + '/site/plan_your_tripOne');
